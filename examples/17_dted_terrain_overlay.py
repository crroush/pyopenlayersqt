...
class DTEDTerrainRenderer(QtWidgets.QMainWindow):
    ...
    def _view_inside_coverage(self, ext: Dict[str, float]) -> bool:
        ...
        return (
            float(ext["lat_min"]) >= (lat_min - eps)
            and float(ext["lat_max"]) <= (lat_max + eps)
            and float(ext["lon_min"]) >= (lon_min - eps)
            and float(ext["lon_max"]) <= (lon_max + eps)
        )
    ...
    def _on_view_extent(self, extent: Dict[str, float]):
        ...
        if key == self._last_requested_key and self._pending_future and not self._pending_future.done():
            self._dbg("skip: same request key while prior request still running")
            return
