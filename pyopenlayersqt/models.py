...
class IconStyle:
    """
    Advanced style options for a point rendered with a custom image icon.

    Most callers should pass ``icon=...`` plus direct scale/anchor options to
    ``VectorLayer.add_icon_points()``. That helper accepts local paths, URLs,
    data URIs, or bytes-like values (including QByteArray) and normalizes them for
    the embedded browser.

    icon_src: Browser-loadable image URL generated by VectorLayer.add_icon_points(),
        or an explicit URL/data URI when using IconStyle directly with add_points().
    selected_icon_src: Optional browser-loadable image URL to use while the
        feature is selected. Prefer passing ``selected_icon=...`` to
        ``VectorLayer.add_icon_points()`` so local files/bytes are normalized.
    ...
    icon_src: str = ""
    selected_icon_src: Optional[str] = None
    scale: float = 1.0
    opacity: float = 1.0
...
class FastPointsStyle:
    ...
    pixel_render_threshold controls when the canvas renderer switches from
    exact circle drawing to one-pixel marker drawing. Pixel rendering still
    evaluates every visible point, but avoids expensive arc drawing for dense
    zoomed-out views.
    """
    radius: float = 3.0
    ...
    pixel_render_threshold: int = 200_000
    ...
    def to_js(self) -> dict:
        ...
        return {
            ...
            "pixel_render_threshold": max(1, int(self.pixel_render_threshold)),
        }
