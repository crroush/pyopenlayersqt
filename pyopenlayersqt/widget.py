...
class OLMapWidget(QWebEngineView):
    ...
    def _icon_to_src(self, icon: Any) -> str:
        """Normalize a path, URL, data URI, or bytes-like icon to a browser URL."""
    ...
    def set_base_visible(self, visible: bool) -> None:
        ...
        self.send({"type": "base.set_visible", "visible": self._show_osm_layer})

    def set_map_background_color(self, color: str) -> None:
        """Set the map background color (shown behind OSM tiles)."""
    ...
    def get_view_extent(self, callback):
        ...
        extent_dict contains lon_min, lat_min, lon_max, lat_max,
        zoom, resolution, width_px, height_px.
    ...
    def add_tile_layer(
        self, opt: Optional[TileLayerOptions] = None, name: str = "tile"
    ) -> TileLayer:
        layer_id = self._next_id("t")
        opt = opt or TileLayerOptions()
        self._send(
            {
                "type": "layer.add_tile",
                "layer_id": layer_id,
